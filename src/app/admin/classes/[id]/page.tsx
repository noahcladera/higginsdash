...
export default async function EditClassPage({
  ...
  // Persisted multi-group flag — when only the synthetic default group
  // exists, the series is a "regular" pickup class and renders the
  // flat coach UI / hides the Sub-groups card. As soon as the admin
  // adds a second group we automatically upgrade to the lead+assistants
  // and per-group coach UI.
  const isMultiGroup = visibleGroups.length >= 2;
  const useSplitGroupsUI = youthPickup && isMultiGroup;
  ...
  return (
    <div className="space-y-8">
      ...
      <SectionCard
        ...
        description={
          useSplitGroupsUI
            ? `One lead ${terms.coach.singular.toLowerCase()} plus any assistants. Per-session substitutions live on each session row below.`
            : `Who is teaching this ${terms.class.singular.toLowerCase()}. Per-session substitutions live on each session row below.`
        }
        ...
        read={
          <CoachesReadout
            ...
            isPickup={useSplitGroupsUI}
            simple={!useSplitGroupsUI && !isEvent}
          />
        }
        edit={
          isEvent ? (
            ...
            <EventCoachesSectionEditor
              ...
              coaches={coaches}
              defaultPersonIds={eventStaffIds}
              memberLabel={terms.coach.singular}
              addAnotherLabel={`Add another ${terms.coach.singular.toLowerCase()}`}
            />
          )
        }
      />
...
function CoachesReadout({
  ...
  isPickup: boolean;
  /** Flat list without lead/assistant labels (adult and non-pickup youth). */
  simple?: boolean;
}) {
  const real = assignments.filter(
    (a) => a.coachPersonId !== SYSTEM_NO_COACH_PERSON_ID,
  );
  ...
  if (simple) {
    return (
      <div className="text-sm">
        {real.length === 0 ? (
          <span className="text-[var(--muted-foreground)]">No coaches yet.</span>
        ) : (
          <span className="font-medium">{real.map((a) => a.name).join(", ")}</span>
        )}
      </div>
    );
  }
