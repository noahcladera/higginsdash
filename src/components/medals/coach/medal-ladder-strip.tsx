...
export function MedalLadderStrip({
  ...
  return (
    <>
          ...
          {MEDAL_LEVELS.map((level) => {
            ...
            return (
              <GroupedLinkRow
                ...
                className={cn(
                  "gap-3",
                  isActive && "bg-[var(--triaz-soft)]/40",
                )}
              >
                <span
                  className={cn(
                    "inline-flex shrink-0 rounded-md px-2 py-0.5 text-xs font-bold",
                    chip,
                  )}
                >
                  {level.shortCode}
      ...
      <div className={cn("hidden overflow-x-auto pb-2 lg:block", className)}>
